...
class Mahasiswa extends Database {
    ...
    public function inputMahasiswa($data){
        ...
        $query = "INSERT INTO tb_mahasiswa (nim_mhs, nama_mhs, ranking_mhs, alamat, provinsi, email, telp, status_mhs) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        $stmt->bind_param("ssssssss", $nim, $nama, $ranking, $alamat, $provinsi, $email, $telp, $status);
        $result = $stmt->execute();
        $stmt->close();
    ...
    public function getAllMahasiswa(){
        // Menyiapkan query SQL untuk mengambil data mahasiswa beserta ranking dan provinsi
        $query = "SELECT id_mhs, nim_mhs, nama_mhs, nama_ranking, nama_provinsi, alamat, email, telp, status_mhs 
                  FROM tb_mahasiswa
    ...
    public function editMahasiswa($data){
        ...
        $query = "UPDATE tb_mahasiswa SET nim_mhs = ?, nama_mhs = ?, ranking_mhs = ?, alamat = ?, provinsi = ?, email = ?, telp = ?, status_mhs = ? WHERE id_mhs = ?";
        $stmt = $this->conn->prepare($query);
        if(!$stmt){
            return false;
        }
        // Memasukkan parameter ke statement
        $stmt->bind_param("ssssssssi", $nim, $nama, $ranking, $alamat, $provinsi, $email, $telp, $status, $id);
        $result = $stmt->execute();
        $stmt->close();
        // Mengembalikan hasil eksekusi query
        return $result;
